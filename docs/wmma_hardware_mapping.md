...
`examples/device_query.cu` 是一个完整的查询工具：

```bash
nvcc -o device_query examples/device_query.cu
./device_query
```
...
以下内容以 **RTX 3080 (Compute 8.6)** 为例，根据硬件参数推导代码设计参数。
...
**设计决策**：`WARP_ROW_TILES=2, WARP_COL_TILES=2`（V2-V4）是一个折中配置。累加器用 32 寄存器，加上其他开销约 50-64，occupancy 约 66-100%。
