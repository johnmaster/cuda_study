...
![GPU/CPU 异构系统结构](../images/gpu-cpu-system-diagram.png)
...
![CUDA 内存架构](../images/cuda_memory_architecture.png)
...
![Shared Memory Bank](../images/shared_memory_bank.png)
...
* **后续架构（如 Blackwell）**：在 **TMA、异步内存、新 Tensor Core 指令** 上继续演进；具体行为以 **当前 toolkit 的 Programming Guide + PTX ISA** 为准，**不可**假定 Hopper 代码零修改即得最优点。

**架构关系**：**Ampere `cp.async` 提供细粒度异步 Global→Shared；Hopper+ 通过 `mbarrier`、Bulk、`TMA` 将完成者、等待者和传输粒度进一步硬件化，更适合大 tile GEMM。**
...
![H100 Streaming Multiprocessor](../images/H100-Streaming-Multiprocessor-SM-625x869.png)
