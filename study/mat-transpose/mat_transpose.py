...
def run_benchmark(
    ...
    out_info = f"out_{tag}"
    # out.T对张量out进行转置
    # .equal(x)比较out.T和x是否完全相同
    real_t = f"{out.T.equal(x)}"
    # out[:2, :2]取张量out的前2行，前2列
    # flatten()把这个张量拉成一维
    # detach()切断梯度跟踪
    # cpu()把张量从gpu转移到cpu
    # numpy()转换成numpy数组
    # tolist()转换成python的list
...
for M, N in MNs:
    print("-" * 130)
    print(" " * 55 + f"M={M}, N={N}")
    # torch.arange(0, M * N)生成从0到M*N-1的整数序列
    # reshape生成形状为(M, N)的矩阵
    ...
    if M == N:
        run_benchmark(lib.mat_transpose_f32_diagonal2d, x, "f32_diagonal", y)
